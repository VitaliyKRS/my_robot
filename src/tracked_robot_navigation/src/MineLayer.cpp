...
void MineLayer::onInitialize()
{
    ...
    mMinePositionSubscription = node->create_subscription<geometry_msgs::msg::Point>(
        "/mine_position", 10,
        std::bind(&MineLayer::onMinePositionReceived, this, std::placeholders::_1));
}

void MineLayer::updateBounds(double robot_x,
                             double robot_y,
                             double robot_yaw,
                             double* min_x,
                             double* min_y,
...
void MineLayer::updateCosts(nav2_costmap_2d::Costmap2D& master_grid,
                            int min_i,
                            int min_j,
                            int max_i,
                            int max_j)
    ...
    for (const auto& mineObstacle : mMineObstacles) {
        ...
        for (int dx = -radius; dx <= radius; ++dx) {
            for (int dy = -radius; dy <= radius; ++dy) {
                ...
                if (neighbor_x >= 0 && neighbor_x < map_size_x && neighbor_y >= 0 &&
                    neighbor_y < map_size_y) {
...
bool MineLayer::isClearable() { return true; }
void MineLayer::reset() { mMineObstacles.clear(); }
...
void MineLayer::onMinePositionReceived(const geometry_msgs::msg::Point::SharedPtr minePos)
{
    ...
    if (std::none_of(mMineObstacles.begin(), mMineObstacles.end(), [minePos](auto& point) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("MineLayer"), "Mine received %f, %f!", minePos->x,
                    minePos->y);
