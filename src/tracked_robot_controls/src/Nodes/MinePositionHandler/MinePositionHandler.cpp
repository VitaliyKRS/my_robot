    ...
    , mHandLenght{0.7}
{
    ...
    mRobotPositionSubscription = this->create_subscription<nav_msgs::msg::Odometry>(
        "/odom", 10,
        std::bind(&MinePositionHandler::onOdometryReceived, this, std::placeholders::_1));
    ...
    mMinePositionPublisher =
        this->create_publisher<geometry_msgs::msg::Point>("/mine_position", 10);
}
...
void MinePositionHandler::publishMinePosition()
{
    ...
    // Transform the mine's relative coordinates to the map frame using the robot's position and
    // orientation
    ...
    if (std::sqrt((pointMsg.x - mOldPosition.x) * (pointMsg.x - mOldPosition.x) +
        ...
        RCLCPP_INFO(this->get_logger(), "Mine position - x{%f}, y{%f}", transformedMineX,
                    transformedMineY);
