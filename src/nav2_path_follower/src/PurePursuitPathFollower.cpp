...
void PurePursuitPathFollower::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent,
                                        std::string name,
                                        ...
                                        std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    declare_parameter_if_not_declared(node, plugin_name_ + ".back_distance",
                                      rclcpp::ParameterValue(1.2));
    declare_parameter_if_not_declared(node, plugin_name_ + ".backward_speed",
                                      rclcpp::ParameterValue(2.0));
    declare_parameter_if_not_declared(node, plugin_name_ + ".max_angle_difference",
                                      rclcpp::ParameterValue(0.5));
    declare_parameter_if_not_declared(node, plugin_name_ + ".mine_pos_topic",
    ...
    mMinePositionSubscription = node->create_subscription<geometry_msgs::msg::Point>(
        mMinePosTopic, 10,
        std::bind(&PurePursuitPathFollower::onMinePositionReceived, this, std::placeholders::_1));
}
...
void PurePursuitPathFollower::activate()
{
    Parent::activate();
    auto node = node_.lock();
    mDynParamHandler = node->add_on_set_parameters_callback(std::bind(
        &PurePursuitPathFollower::dynamicParametersCallback, this, std::placeholders::_1));
}
...
geometry_msgs::msg::TwistStamped PurePursuitPathFollower::computeVelocityCommands(
...
void PurePursuitPathFollower::setPlan(const nav_msgs::msg::Path& path) { Parent::setPlan(path); }
...
rcl_interfaces::msg::SetParametersResult PurePursuitPathFollower::dynamicParametersCallback(
...
void PurePursuitPathFollower::onMinePositionReceived(
    const geometry_msgs::msg::Point::SharedPtr minePos)
{
...
geometry_msgs::msg::TwistStamped PurePursuitPathFollower::computeBackWardVelocityCommand(
    const geometry_msgs::msg::PoseStamped& pose)
{
    geometry_msgs::msg::TwistStamped cmd_vel;
    ...
    if (!mMinePositions.empty()) {
        for (const auto& minePos : mMinePositions) {
            double distance_to_mine =
                std::hypot(pose.pose.position.x - minePos.x, pose.pose.position.y - minePos.y);

            double angle_to_mine =
                atan2(minePos.y - pose.pose.position.y, minePos.x - pose.pose.position.x);
...
PLUGINLIB_EXPORT_CLASS(nav2_path_follower::PurePursuitPathFollower, nav2_core::Controller)
